...
(async()=>{
  ...
  await page.getByTestId('stSidebar').getByTestId('stRadioOption').filter({hasText:'Sales'}).click();
  await page.getByText('Customer quotations and orders',{exact:true}).click();
  await page.getByRole('button',{name:/Customer orders$/}).click();
  await page.getByRole('heading',{name:'Customer quotations and sales orders'}).waitFor();
  await page.screenshot({path:path.join(output,'customer-orders.png'),fullPage:true});
