...
(async()=>{
  ...
  await page.getByTestId('stSidebar').getByTestId('stRadioOption').filter({hasText:'Manufacturing and jobs'}).click();
  await page.getByRole('heading',{name:'Manufacturing and job work'}).waitFor();
  ...
  await page.getByRole('button',{name:'Record production',exact:true}).waitFor();
