...
st.caption('Identify material shortages from confirmed customer commitments and open work orders.')
...
b.metric('Products needing production',len({r['product_id'] for r in plan if r['shortage'] and r['action']=='Make'}))
...
rows=[{'Product':r['product'],'SKU':r['item_code'],'Demand source':r['source'],'Required by':r['due_date'],'Unit':r['unit'],
    ...
    'Scheduled production':quantity(r['scheduled_output']),'Shortage':quantity(r['shortage']),'Action':r['action'],
...
with st.expander('How this plan is calculated'):
    ...
    st.write('This is a material plan, not a machine-capacity schedule. All internal warehouses are pooled; arrange transfers to the production location. Work-order materials are planned by the work-order due date. Purchases with no delivery date appear separately and do not reduce shortages.')
    st.write('Create purchase drafts on Purchase orders and production plans on Work orders after reviewing these recommendations.')
