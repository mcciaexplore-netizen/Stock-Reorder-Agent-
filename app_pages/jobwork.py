...
st.title('Outside work tracking')
st.caption('Track ownership, outward and inward records, partial returns and stock still held outside your location.')
customers={c['id']:c for c in store.customers()}
with st.expander('Open an outside-work order'):
    with st.form('jw_create'):
        ...
        party=st.text_input('Outside party / processing site')
        ...
        ref=st.text_input('Customer / outside-work reference')
        submit=st.form_submit_button('Create outside-work order',disabled=not store.allowed('inventory'))
    if submit:act('jw_create',lambda:store.create_jobwork(owner,cid,party,lid,due.isoformat(),ref,actor=actor,token=operation_key('jw_create')),'Outside-work order opened.')
orders=store.jobwork_orders()
if not orders:st.info('Open an outside-work order to receive customer material or send company material.'); st.stop()
grid([{'Outside work':j['number'],'Material owner':j['customer'] if j['owner']=='customer' else 'Company','Outside party':j['subcontractor'],
    'Return due':j['due_date'],'Status':j['state'],'Overdue':j['state']=='open' and j['due_date']<date.today().isoformat()} for j in orders])
lookup={j['id']:j for j in orders}; jid=st.selectbox('Open outside-work order',list(lookup),format_func=lambda i:lookup[i]['number']); j=lookup[jid]
...
if j['state']=='open' and products:
    with st.expander('Receive customer material' if j['owner']=='customer' else 'Send company material outside'):
        pid=pick_product('Outside-work material',key='jw_product'); batch=pick_batch(pid,j['location_id'],key='jw_batch')
    ...
    if j['state']=='open':
        ...
        with st.expander('Partial return / material reconciliation',expanded=True):
            with st.form(f'jw_reconcile_{jid}'):
                ...
                submit=st.form_submit_button('Record outside-work movement',disabled=not store.allowed('inventory'))
            if submit:act('jw_reconcile',lambda:store.reconcile_jobwork(mid,action,qty,ref,actor=actor,token=operation_key('jw_reconcile')),'Outside-work balances and challan updated.')
...
if challans:
    with st.expander('Challan history and downloads'):
        ...
        st.download_button('Export reconciliation history',csv_bytes(challans),'outside-work-history.csv','text/csv')
if j['state']=='open' and st.button('Close fully reconciled outside work',disabled=not store.allowed('inventory')):
    act('jw_close',lambda:store.close_jobwork(jid,actor=actor),'Outside-work order closed.')
