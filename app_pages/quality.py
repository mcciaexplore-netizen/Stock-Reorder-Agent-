st.title('Quality inspections')
st.caption('Hold material for inspection, record acceptance or rejection, and release reworked stock after reinspection.')
if products:
    with st.expander('Start incoming / production / final inspection'):
        pid=pick_product('Material to inspect',key='qc_product')
        ...
        with st.form(f'qc_open_{pid}_{lid}_{batch}'):
            stage=st.selectbox('Inspection stage',['incoming','production','final'])
            qty=st.text_input('Quantity to quarantine',value='1')
            ref=st.text_input('Receipt / production / dispatch reference')
            submit=st.form_submit_button('Hold stock for inspection',disabled=not store.allowed('inventory'))
...
if not checks:st.info('Incoming receipts can be held for inspection. Work-order output enters final inspection automatically.'); st.stop()
...
lookup={q['id']:q for q in checks}; qid=st.selectbox('Open quality inspection',list(lookup),format_func=lambda i:f'QC-{i} · {lookup[i]["item_name"]}')
q=lookup[qid]
if q['state'] in ('awaiting','rework'):
    with st.form(f'qc_result_{qid}_{q["accepted"]}'):
        ...
        notes=st.text_area('Inspection observations / measurements')
        submit=st.form_submit_button('Save inspection and release accepted stock',disabled=not store.allowed('inventory'))
    if submit:act('qc_result',lambda:store.record_quality_result(qid,accepted,rejected,notes,actor=actor,token=operation_key('qc_result')),'Inspection recorded. Accepted stock released; rejected stock remains blocked.')
if q['state']=='rejected':
    ...
    if submit:act('qc_rework',lambda:store.route_quality_rework(qid,notes,actor=actor),'Rework recorded. Complete reinspection before release.')
...
if history:
    ...
    grid([{'Date':r['created_at'],'Action':r['action'],'Accepted':quantity(r['accepted']),'Rejected / disposed':quantity(r['rejected']),'Observations':r['notes'],'Inspector':r['actor']} for r in history])
